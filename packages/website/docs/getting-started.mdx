...
const productsPluginWithoutKeyboardNavigation = createProductsPlugin({
  transformSource(source) {
    const { getItemUrl, ...rest } = source;
    return rest;
  },
});
...
<AutocompleteExample plugins={[productsPluginWithoutKeyboardNavigation]} />
...
<AutocompleteExample plugins={[productsPlugin]} />
