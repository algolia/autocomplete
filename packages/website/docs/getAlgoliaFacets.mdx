...
> `"default" | "facet"` | required

The type of query to perform. When using `getAlgoliaFacets`, this should be set to `"facet"`.
...
Note that for this to work, it must be declared in the [`attributesForFaceting`](https://www.algolia.com/doc/api-reference/api-parameters/attributesForFaceting/) index setting with the `searchable()` modifier.

#### `params`

> [`SearchOptions`](https://www.algolia.com/doc/api-reference/search-api-parameters/)
...
The function to transform the Algolia response before passing it to the Autocomplete state. You have access to the full Algolia results, as well as the pre-computed hits and facet hits. This is useful to manipulate the hits, or store data from the results in the [context](context).
